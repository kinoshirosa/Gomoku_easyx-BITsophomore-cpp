...
piece_color game() {
	...
	// board initiative module
		...
		board.print_board_easyx(interval, instruct_size, default_line_color);
	while (1) {
	// coordination input module: can be adjust to easyx module or windows module
		...
		// input from textbox
			if (!button_confirm.run(msg)) {
				confirm_negedge_indicator = true;	// making confirm working when clicked
			}
...
void flush_board(Board& board, Piece& new_piece) {	// 将键盘信息刷新到界面，但是因为是遍历，所以要少用
	for (int i = 0; i < default_size; i++) {
		for (int j = 0; j < default_size; j++) {
