...
void Board::print_board_easyx(const int& interval, const int& instruct_size, const COLORREF& bk_color,  const COLORREF& line_color) {
	int length = interval * _size;
	setfillcolor(bk_color);
	solidrectangle(0, 0, length, length);
...
void Board::print_piece_easyx(const int& interval, const int& instruct_size,
	...
	if (current_color == BLACK_PIECE) {
		setfillcolor(BLACK);
		solidcircle(center_x, center_y, piece_size - 1);
	}
...
void Board::erase_piece_easyx(const int& interval, const int& instruct_size, const int& piece_size,
	const int& x_input, const int& y_input,
	const COLORREF bk_color, const COLORREF line_color) {
	int center_x = interval * x_input;
	int center_y = interval * y_input;
	setfillcolor(bk_color);
	solidcircle(center_x, center_y, piece_size);
	setlinecolor(line_color);
	line(center_x - piece_size, center_y, center_x + piece_size, center_y);
	line(center_x, center_y - piece_size, center_x, center_y + piece_size);
}